...
public interface ReservationControllerApi {
    ...
    @Operation(
            summary = "예약 수정",
            description = "요청 헤더 `X-Device-Id`로 식별되는 회원이 본인 소유 예약 전체 필드를 교체한다. "
                    + "예약명(nickname), 출/도착지, 경로 옵션, 도착 시각, 반복 요일을 모두 전송한다. 멱등성 보장."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
    ...
    org.example.when2go.global.response.ApiResponse<Void> update(
            @Parameter(
                    name = "X-Device-Id",
                    description = "회원 식별용 디바이스 ID (UUID 36자)",
                    in = ParameterIn.HEADER,
